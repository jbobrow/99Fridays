...
void main( void ) {
	...
	if( brightness < maxThreshold && brightness > minThreshold ) {//isPersonPresent) {
		...
		float sum = 0.;
		sum += texture2D(ppixels, position + pixel * vec2(-1., -1.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(-1., 0.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(-1., 1.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(1., -1.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(1., 0.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(1., 1.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(0., -1.)).r;
		sum += texture2D(ppixels, position + pixel * vec2(0., 1.)).r;
		...
		if (me.r <= 0.1) {
			if ((sum >= 2.9) && (sum <= 3.1)) {
				...
				gl_FragColor = vec4(0., 0., me.b - 0.004, 1.);
			} else {
				gl_FragColor = dead;
			}
		} else {
